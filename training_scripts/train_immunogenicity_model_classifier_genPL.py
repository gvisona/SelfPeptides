...
def train(config=None, init_wandb=True):
    ...
    calib_predictions = []
    for ix, calib_batch in tqdm(enumerate(calib_loader)):
        peptides, hla_pseudoseqs, imm_target = calib_batch
        imm_target = imm_target.float().to(device)
        ...
        if not isinstance(predictions, list):
            predictions = [predictions]
        calib_predictions.extend(predictions)

    calib_predictions.extend([np.nan]*(len(calib_df)-len(calib_predictions)))

    
    calib_df["Prediction"] = calib_predictions
    targets = calib_df["Target"].values
    calib_predicted_proba = sigmoid(np.array(calib_predictions))
    
    n_cal = len(calib_df)
    
    conformal_alpha = config.get("conformal_alpha", 0.1)
    scores = np.abs(targets - calib_predicted_proba)
    q_level = np.ceil((n_cal+1) * (1-conformal_alpha))/n_cal
    q_hat = np.quantile(scores, q_level)
    if q_hat>=0.5:
        print("High Conformal Quantile: CHECK")
    wandb.run.summary["Conformal Q_Hat"] = q_hat
    
            
    ba_dset = SequencesInteractionDataset_returnHLA(ba_df, hla_repr=config["hla_repr"], target_label="Label")
    ba_loader = DataLoader(ba_dset, batch_size=config['batch_size'], drop_last=False, shuffle=False)
    
    print("Calculating pseudolabels..")
    pseudolabels_peptides = []
    pseudolabels_hlas = []
    pseudolabels_hlas_pseqs = []
    pseudolabels_predictions = []
    for ix, pl_batch in tqdm(enumerate(ba_loader)):
        peptides, hla_alleles, hla_pseudoseqs = pl_batch[:3]
        predictions = model(peptides, hla_pseudoseqs).view(-1)
        
        predictions = predictions.detach().cpu().tolist()
        ixs = [i for i, p in enumerate(predictions) if p<q_hat or p>1-q_hat]
        pseudolabels_peptides.extend([peptides[i] for i in ixs])
    ...
    pseudolabeled_data = pd.DataFrame({"Peptide": pseudolabels_peptides, 
                                       ...
                                       "Allele Pseudo-sequence": pseudolabels_hlas_pseqs, 
                                       "Target": np.array(pseudolabels_predictions).astype(int)
                                      })
    pseudolabeled_data.to_csv(join(output_folder, "generated_pseudolabels_set.csv"), index=False)
    print("Generated {} pseudolabels".format(len(pseudolabeled_data)))
