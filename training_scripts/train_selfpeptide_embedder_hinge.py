...
from selfpeptide.utils.training_utils import CustomCosineDistanceHingeLoss
...
def train(config=None, init_wandb=True):
    ...
    train_dset = Self_NonSelf_PeptideDataset(config["hdf5_dataset"], 
                                             gen_size=config["gen_size"], val_size=config["val_size"]+config["ref_size"], 
                                             test_run=config["test_run"])
    
    train_loader = DataLoader(train_dset, batch_size=config["batch_size"], shuffle=False, drop_last=True, num_workers=4)
    ...
    loss_function = CustomCosineDistanceHingeLoss(margin=margin, device=device, reg_weight=config.get("reg_weight", 0.0))
...
if __name__=="__main__":
    ...
    parser.add_argument("--experiment_name", type=str, default="hinge_grad_check")
    ...
    parser.add_argument("--pretrained_aa_embeddings", type=str, default="/home/gvisona/Projects/SelfPeptides/processed_data/aa_embeddings/normalized_learned_BA_AA_embeddings.npy")
